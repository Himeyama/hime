...
- **Web検索**: 「～とは？」や「～教えて」や「調べて」や「検索して」や「最新の～」等の単語が含まれる要求があった場合は、WebSearch や WebFetch を使用して最新の情報を調査します。最新の情報、知識範囲外の事柄、ライブラリのバージョン情報、エラーメッセージの解決策など、外部情報が必要な場合も **WebSearch** や **WebFetch** を使用します。
...
- **Web ページの分析**: ユーザーが URL をプロンプトに含めた場合は、その URL を WebFetch で読み込んでください。
