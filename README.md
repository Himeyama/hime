...
> **AI SDK**: [Vercel AI SDK](https://sdk.vercel.ai/) を使用。全プロバイダーで統一されたストリーミング・ツールコールループを実現しています。
...
| OpenRouter | 必要 | `https://openrouter.ai/api/v1` |
| Google Gemini | 必要 | — |

### Google Gemini の認証

[Google AI Studio](https://aistudio.google.com/) で取得した API キーを設定パネルに入力してください。

> **Vertex AI** を使用する場合は、別途 `@ai-sdk/google-vertex` のインストールとカスタマイズが必要です。
...
- `src/providers/`: AI プロバイダーの実装クラス
  - `vercel-base.ts`: Vercel AI SDK ベースの共通 `chat()` 実装（全プロバイダーが継承）
  - `anthropic.ts`, `openai.ts`, `azure-openai.ts`, `ollama.ts`, `openrouter.ts`, `google.ts`
